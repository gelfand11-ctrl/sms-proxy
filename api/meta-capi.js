...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ADD THIS: always capture real IP server-side, ignore whatever frontend sent
    const finalIp = req.headers['x-forwarded-for']?.split(',')[0].trim() || req.socket.remoteAddress;
    // ADD THIS: fallback to header if frontend didn't send userAgent
      ...
      data: [
        {
          ...
          user_data: {
            ph: sha256(phone),
            client_ip_address: finalIp || undefined, // CHANGED: was "ip"
            client_user_agent: finalUserAgent || undefined, // CHANGED: was "userAgent"
            fbc: fbc || undefined,
            fbp: fbp || undefined,
          },
          custom_data: {
